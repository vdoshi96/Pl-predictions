...
Evidence date: 2026-08-08. This document records the newest completed local and production verification for the seven-category spotlight prediction and reviewed player-catalogue release. GitHub PR #7 is merged and its exact application deployment is live at the stable alias. Two operational controls remain open and are stated as pending below: a deployment after removal of the legacy administrator secret, and owner publication of the staged Web Application Firewall rate limit. Earlier release history remains in `docs/context/LOG.md`.
...
| Vercel production build        | deployment `dpl_5KBCAG4xhrZuapSqMg5LK4i6myGR`                                | Ready with target `production` for exact Git SHA `c976c1719c8c17f8169b9c8b86248765821b203a`. Its immutable URL and the stable alias both resolve to this application release.                                                                                                                                                                                                                                    |
...
| Legacy administrator fallback  | Vercel environment inventory                                                 | `ADMIN_SECRET` was removed from the Vercel environments. The currently inspected deployment predates that removal, so one subsequent deployment is still required before production runtime is described as legacy-free.                                                                                                                                                                                         |
| Administrator login WAF        | Vercel firewall inspection                                                   | The live log-only rule observed exactly one intended login `POST`. A 10-request-per-60-second, per-IP, HTTP-429 rate limit is staged but is not live; owner publication remains required.                                                                                                                                                                                                                        |
...
The first production rerun exposed two harness-only browser boundaries, not product failures. A touch drag may finish without crossing the insertion boundary on its first attempt, so the smoke permits one bounded retry and still requires the club order to change. A browser may cancel a speculative optimized player-face candidate during navigation; the request guard excludes only that cancellation while the UI makes explicit assertions that selected catalogue portraits decode with nonzero natural dimensions and that the custom Other-player fallback renders the silhouette with no image. The corrected read-only production run then passed all five projects.
...
GitHub [PR #7](https://github.com/vdoshi96/Pl-predictions/pull/7) merged the tested feature into the working `main` branch at exact merge SHA `c976c1719c8c17f8169b9c8b86248765821b203a`. Deployment `dpl_5KBCAG4xhrZuapSqMg5LK4i6myGR` is Ready with target `production` at immutable URL [https://pl-predictions-7lhcv198i-vdoshi96s-projects.vercel.app](https://pl-predictions-7lhcv198i-vdoshi96s-projects.vercel.app). Its metadata identifies that exact SHA, and the stable public alias [https://pl-predictions-2026.vercel.app](https://pl-predictions-2026.vercel.app) points to it. This record does not claim that GitHub `main` is the repository's configured default branch.
...
The successful login proves the configured hash path because the application prioritizes `ADMIN_PASSWORD_HASH` when it is present. The legacy `ADMIN_SECRET` name has since been removed from production, preview, and development environment inventories, but the current application deployment was built before that removal. A subsequent deployment and bounded login verification remain required before describing the running deployment as hash-only.

The live WAF rule remained in log mode for the verification and observed exactly the one intended administrator-login `POST`. The prepared change would enforce 10 requests per 60 seconds per IP with HTTP 429 on excess requests, but it remains staged under the owner-publication boundary and is not live.
...
- [x] Verify Ready production deployment `dpl_5KBCAG4xhrZuapSqMg5LK4i6myGR`, its immutable URL, exact SHA, and the stable alias mapping.
...
- [x] Confirm username-plus-PBKDF2 authentication and remove the legacy `ADMIN_SECRET` name from Vercel environment inventories.
- [ ] Deploy after the legacy-secret removal and rerun a bounded owner-login check before claiming the production runtime is legacy-free.
...
- [ ] Complete the evidence-only documentation generation, merge, local `main` synchronization, and worktree cleanup; this record does not claim those steps are already complete.
