...
2. Import the owner-provided `premier-league-players-2026-08-13/` snapshot into the season-scoped player catalogue: 582 players across the 20 clubs and 582 copied local portrait PNGs. Search first, last, or full names, use the `PlayerMark` silhouette only when an asset path is absent or an image fails, and retain Other player plus a required custom-name field for unavailable or newly added players. Continue using existing local club crests for the three club categories.
