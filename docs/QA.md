...
Evidence date: 2026-08-08. This document records the newest completed local and production verification for the owner-provided club badge iteration. The feature is merged, the PNG deployment is live at the stable alias, all 20 production asset paths are migrated, and the mobile evidence has been refreshed. Prior-release write and cleanup evidence is retained below as intentional history.
...
| Vercel production build           | deployment `dpl_2z72V3tGKDssEAggjqMEH4PQQsYN`  | Ready with target `production` for GitHub merge `1c8a20b`; the stable alias points to it, and both the home page and raw PNG asset returned 200 before the production seed.                                              |
| Anonymous production smoke        | `npm run test:production-smoke`                | 5 passed against the stable alias across desktop Chromium, mobile Chromium at 390 pixels, exact 320/430-pixel reflow projects, and mobile WebKit. All prediction and review-dialog marks loaded from PNG sources.        |
| Headed production badge check     | stable alias, desktop and narrow Chromium      | 20 of 20 marks decoded from PNG sources, no initials fallback appeared, all 20 optimized requests returned 200, the console had zero messages, and no horizontal overflow occurred.                                      |
| Bounded production write smoke    | `npm run test:production-write-smoke`          | Not run for this badge iteration. Historical prior-release evidence: 1 passed with an exact QA submit/privacy/delete journey and exact cleanup.                                                                          |
| Production data mutation          | explicitly approved idempotent seed            | Asset paths changed from 20 SVG/0 PNG to 0 SVG/20 exact PNG fixture matches. Non-path team metadata was unchanged; prediction, standings-snapshot, and standings-item counts each stayed zero.                           |
| Vercel runtime logs               | exact deployment, newest 500 within 30 minutes | All 500 records were HTTP 200; zero warnings, errors, 4xx, or 5xx responses.                                                                                                                                             |

The local sandbox did not permit a default Turbopack CSS helper to bind its internal port, so the local production gate used `next build --webpack`. This was an execution-environment restriction, not an application compile error; Vercel deployment `dpl_2z72V3tGKDssEAggjqMEH4PQQsYN` subsequently passed the production build.
...
The first final aggregate rerun exposed a local WebKit timing boundary: its five-second success-message wait expired while the standings server action was still pending, then the request completed and the retry correctly returned a duplicate. Exact isolated cleanup restored the null active snapshot and accepted-through watermark, deleted snapshot `9ca94c9f-16c2-51b2-ae1a-de347fcd8aaf`, import runs `400c1cb5-58e8-48a3-8cb6-921ae73ce01e` and `4055de69-2589-4185-accc-5abcbef5b65d`, and audit rows `8301ddc6-388f-48c1-9eac-e4c916828a44` and `6a4ace78-e9e3-4fe3-877a-06c6d1573602`. The scoped UI wait is now 20 seconds. A targeted WebKit rerun passed in 13.3 seconds, the complete aggregate rerun passed all 8 routed journeys with 22 intentional skips, and the final query found zero snapshots, runs, audits, or `Mobile QA` predictions with all season flags/pointers restored.
...
GitHub [PR #5](https://github.com/vdoshi96/Pl-predictions/pull/5) merged the tested badge feature into canonical `main` at `1c8a20bc0a689f39d5cb392753d3582aa616bdbc`. Deployment `dpl_2z72V3tGKDssEAggjqMEH4PQQsYN` is Ready with target `production` at immutable URL [https://pl-predictions-6hs8vokup-vdoshi96s-projects.vercel.app](https://pl-predictions-6hs8vokup-vdoshi96s-projects.vercel.app). The stable public alias was explicitly approved for repointing, resolves to that exact deployment, and returned 200 for both the home page and raw Arsenal PNG before the database transition. The project remains `vdoshi96s-projects/pl-predictions` with Marketplace Neon resource `neon-coffee-queen`. The final evidence-only closeout deployment is intentionally not pinned here because it changes no runtime application behavior.
...
The final read-only browser run against the stable alias passed all five production projects: desktop Chromium, touch-enabled mobile Chromium at 390 pixels, exact 320- and 430-pixel Chromium reflow, and mobile WebKit. It verified application and health responses, no document-level horizontal overflow, real mouse/delayed-touch/keyboard reordering, exactly 20 PNG sources, and nonzero natural widths for all prediction and review-dialog club marks. Safari-style browser cancellation of a Next Image candidate is excluded only when it is a local `/team-marks/` optimizer candidate; the rendered mark must still complete successfully, while genuine request failures and every HTTP 4xx/5xx response remain failures. An independent headed run found 20 loaded PNG marks, no initials fallbacks, 20 optimized 200 responses, a silent console, and no overflow at desktop or narrow width. The exact deployment's newest 500 log records in the inspected 30-minute window were all HTTP 200, with zero warnings, errors, 4xx, or 5xx responses.

After the PNG deployment was live, the owner explicitly approved the idempotent production seed. The active season changed from 20 SVG/0 PNG paths to 0 SVG/20 PNG paths with exact fixture matches and no other path type. The before/after non-path team metadata digest remained `dc907bd0e26186a76d62210381ee593f44225f7c0557f8cede6d43c7dfae7910`; prediction, standings-snapshot, and standings-item counts each stayed zero. No participant or standings write smoke ran for this badge iteration. The separately gated prior-release write smoke remains intentional historical evidence: it passed one exact-ID submit/privacy/delete journey, and its post-cleanup queries returned zero `predictions`, zero `prediction_items`, and zero prediction-target audit rows with the season open, unrevealed, and unlocked and both active and final standings pointers null.
...
Only the newest completed production run is retained. All four screenshots were refreshed at 390 by 844 pixels and visually inspected for the Dranx identity, painted PNG badges in both prediction surfaces, clean narrow-screen rendering, readable wrapping, reachable safe-area actions, and absence of horizontal clipping.

**Prediction entry — mobile Chromium at 390 pixels:** the owner-provided club badges are legible beside long and short club names; the single-column sorter, dedicated move handles, and safe-area-aware review action fit the viewport.

![Production prediction entry on mobile Chromium](assets/qa/prediction-mobile.png)

**Review dialog — mobile Chromium at 390 pixels:** the smoke waits for all 20 dialog badges to paint before restoring the list to the top and capturing evidence; the immutable review remains legible and keeps its confirmation actions reachable without horizontal overflow.
...
GitHub [PR #5](https://github.com/vdoshi96/Pl-predictions/pull/5) merged the tested badge feature into canonical `main` at `1c8a20bc0a689f39d5cb392753d3582aa616bdbc`; its Vercel status was successful. Final closeout leaves local and remote `main` synchronized, no completed badge branch or temporary worktree retained, and one primary worktree. The evidence-only closeout merge SHA is intentionally not pinned here so this end-state record remains accurate.

- [x] Deploy the 20 owner-provided PNG badges before changing persisted paths, verify Ready deployment `dpl_2z72V3tGKDssEAggjqMEH4PQQsYN`, and repoint the stable alias with explicit owner approval.
- [x] Run the explicitly approved idempotent seed and prove exactly 20 SVG paths became 20 exact PNG fixture matches while non-path metadata and participant/standings record counts stayed unchanged.
- [x] Pass the read-only anonymous production smoke in all five projects, independently confirm 20 optimized badge responses and no fallback/console/overflow issue, and inspect the exact deployment's newest 500 log records.
- [x] Retain only the newest labeled mobile prediction, review, leaderboard, and administrator screenshots under `docs/assets/qa/`, with all displayed badges painted before capture.
- [x] Publish the badge evidence through canonical `main`, synchronize local `main` with `origin/main`, remove completed badge branch/worktree state, and retain only the primary worktree.
