...
- GitHub: public owner-supplied repository [vdoshi96/Pl-predictions](https://github.com/vdoshi96/Pl-predictions); `main` is the canonical release branch and local/remote release state is synchronized by the repository closeout workflow.
...
- Remaining closeout: obtain the one explicit Vercel protection approval, run the anonymous read-only and bounded exact-ID write smokes, confirm cleanup/console health, retain newest annotated screenshots, then refresh documentation evidence.
