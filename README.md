...
Deployment status: production is public and Ready at the stable alias above. The August 13 roster release was verified on Vercel deployment `dpl_2CAJEZ3cXpm7NU6K972sN31bVBkz`, whose immutable URL is [https://pl-predictions-835vjpg3z-vdoshi96s-projects.vercel.app](https://pl-predictions-835vjpg3z-vdoshi96s-projects.vercel.app); at release verification time, the stable alias resolved to that deployment ID. The later documentation-only PR #16 merge reached production Ready as `dpl_9bUTkvP7x3Sr55GPMLmDyyuGVMv3` (`mr5jtzypm`) and returned HTTP 200, while the stable alias intentionally remained on the previously verified roster deployment; no alias mutation ran for the documentation closeout. Vercel Authentication is set to `preview`, so retained preview deployments still require owner sign-in while production remains anonymously accessible. Production browser and completed repository-closeout evidence are recorded in [docs/QA.md](docs/QA.md).
