...
4. Derive the 5–3–1 table score and 20-to-1 spotlight rank points on read. Cap the full table at 100, all spotlight picks at 140, and the combined leaderboard at 240. Treat the position-1 champion as part of table scoring, not a separate bonus.
...
6. Keep the five non-table-derived category outcomes explicitly pending until a permitted, reviewed source-neutral result path is designed. Treat the dated roster snapshot as selector identity and portrait input only, not as a goals, assists, clean-sheets, or season-rating feed. Do not add runtime FotMob access, a production scraper, or cron. Ensure custom player names are reconciled before scoring.
7. Preserve pre-reveal privacy for prediction IDs, positions 2–20, and all seven spotlight picks. Add `/rules`, split real leaderboard table/spotlight totals, and show a clearly labelled in-memory test run that cannot affect stored submissions or rankings.
