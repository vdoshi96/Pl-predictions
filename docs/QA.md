...
Evidence date: 2026-08-08. This document records the newest completed local run and the newest completed production baseline. Local evidence covers the Dranx Prediction League iteration; production rows and screenshots remain the immediately preceding release until the Dranx deployment is verified.
...
| Vercel production build           | deployment `dpl_JCfjcxvtVVfbsU55mmTppBK2FB9G` | Previous release baseline: Ready after the project framework preset was corrected from null to `nextjs`; the stable alias points to this deployment and responds anonymously with 200.                                   |
| Anonymous production smoke        | `npm run test:production-smoke`               | Previous release baseline: 5 passed across desktop Chromium, mobile Chromium at 390 pixels, exact 320/430-pixel reflow projects, and iPhone WebKit.                                                                      |
