...
- Publish a manual result only after permanent closure/reveal, at least one active bracket, relevant pinned Other aliases, and explicit owner attestation. Goals, assists, and clean sheets require exact current-`N` coverage including boundary ties. `player_ratings` must contain exactly every distinct player picked for either opinion-player category. A picked opinion player without a reviewed rating stays pending. After attested coverage, omitted canonical subjects in the other factual datasets are outside-range zero and still count as available.
